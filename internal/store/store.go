...
var (
	...
	ErrBonusTokenNotFound     = errors.New("bonus token not found")
	ErrBonusTokenUsed         = errors.New("bonus token already used")
...
// BonusToken is a single-use secret command generated by the bot owner.
...
SELECT COUNT(*)
FROM draws
WHERE chat_id=$1 AND dt=$2 AND manual=TRUE AND bonus=FALSE`, chatID, dt).Scan(&count); err != nil {
			return Winner{}, err
		}
...
// CreateBonusToken stores a secret single-use command. The caller is responsible
// for generating an unguessable token value.
func (s *Store) CreateBonusToken(ctx context.Context, token string, expiresAt time.Time) error {
	_, err := s.db.ExecContext(ctx, `
INSERT INTO bonus_draw_tokens(token, expires_at)
VALUES($1, $2)`, token, expiresAt)
	return err
}

// PickBonusWinner atomically validates and consumes a bonus token, then creates
// one extra manual draw. Bonus draws do not count toward the normal five /force
// calls per day because they are stored with bonus=TRUE.
func (s *Store) PickBonusWinner(ctx context.Context, chatID int64, dt, token string, usedBy int64) (Winner, error) {
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return Winner{}, err
	}
...
FROM bonus_draw_tokens
WHERE token=$1
FOR UPDATE`, token).Scan(&expiresAt, &usedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return Winner{}, ErrBonusTokenNotFound
	}
	if err != nil {
		return Winner{}, err
	}
	if usedAt.Valid {
		return Winner{}, ErrBonusTokenUsed
	}
	if !expiresAt.After(time.Now()) {
		return Winner{}, ErrBonusTokenExpired
	}

	rows, err := tx.QueryContext(ctx, `
SELECT telegram_id,chat_id,username,first_name,last_name,is_admin,active
FROM users
WHERE chat_id=$1 AND active=true`, chatID)
	if err != nil {
		return Winner{}, err
	}
	defer rows.Close()
	...
	var drawID int64
	if err := tx.QueryRowContext(ctx, `
INSERT INTO draws(chat_id,dt,telegram_id,text,manual,bonus)
VALUES($1,$2,$3,'',TRUE,TRUE)
RETURNING id`, chatID, dt, winner.TelegramID).Scan(&drawID); err != nil {
		return Winner{}, err
	}
	if _, err := tx.ExecContext(ctx, `
UPDATE bonus_draw_tokens
SET used_at=NOW(), used_chat_id=$1, used_by=$2
WHERE token=$3`, chatID, usedBy, token); err != nil {
		return Winner{}, err
	}
	if err := tx.Commit(); err != nil {
		return Winner{}, err
	}
	return Winner{ID: drawID, Date: dt, User: winner}, nil
}
